...
plant0 is an tamarindeiro start: 800, stop: 750, water amount: 10000
plant1 is an Abacaxi      start: 700, stop: 650, water amount: 6000 
plant2 is an Jukka        Start: 800, stop 750, water amount: 20000
...
#define averageWaittimesensor 200 // Wait time between each sample
#define globalWaittime 100000 // Wait time after checked all plants and watered every plant. 1000000 = 16 minutes

//Plants setup
#define numberOfPlants 2 // Enter how many plants you have

int sensorPower[numberOfPlants] = {8,9}; // each sensor is controlled individually. Probably will change this later to be a single power signasl
int StartwateringPlant[numberOfPlants] = {800,800}; // Add a new value for each plant, this is when it starts watering
int StopwateringPlant[numberOfPlants] = {750,750}; // Add a new value for each plant, this is when it stop watering.
int SensorpinPlant[numberOfPlants] = {A8,A9}; // Analoge pin, Add a new value for each plant
int SolenoidoutputPlant[numberOfPlants] = {2,3}; // Digital pin, Add a new value for each plant.
int WateringtimePlant[numberOfPlants] = {6000, 6000}; // Divide by 1000 and get seconds. Add a new value for each  plant.
...
void loop() {
  ...
  {
    digitalWrite(sensorPower[PlanteLoop], HIGH); // Turns output low when measuring
  }
   delay(3000);
